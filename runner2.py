import logging
import json
from selectkmeanspp import MRKMeansPlusPlus
...
def run_select_k_random_instances(input_file, k, initial_centroids_file, runner):
    logger.info(
        "Starting MRJob to select %d instances using KMeans++ %s", k, input_file
    )

    # Configure the MRJob runner
    mr_job = MRKMeansPlusPlus(args=[input_file, "--k", str(k), f"--runner={runner}"])
    ...
    while i < iterations:
        ...
        with mr_job.make_runner() as runner:
            ...
            if sorted(centroids) == sorted(new_centroids):
                logger.info("Centroids have converged. Exiting loop.")
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--iterations", type=int, default=5, help="Number of K-Means iterations"
    )
    ...
    K-MEANS ++  file={args.input_file} , k={args.k} , runner={args.runner} , iterations={args.iterations}     
                """)
    ...
    # Step 2: Run the MRKMeans job to perform K-Means clustering
